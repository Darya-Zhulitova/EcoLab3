 ...
 *
 * <сводка>
 *   Функция Fire_OnCombSortIterationStart
 * </сводка>
 *
 * <описание>
 *   Функция вызова обратного интерфейса
 ...
 *
 */
static int16_t ECOCALLMETHOD CEcoLab1_Fire_OnCombSortIterationStart(/* in */ struct IEcoLab1* me, /* in */ size_t n, /* in */ int32_t iteration, /* in */ size_t gap) {
    CEcoLab1* pCMe = (CEcoLab1*)me;
    int16_t result = 0;
    IEcoEnumConnections* pEnum = 0;
    IEcoLab1Events* pIEvents = 0;
    ...
    if (pCMe->m_pISinkCP != 0) {
        ...
        if ( (result == 0) && (pEnum != 0) ) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                result = cd.pUnk->pVTbl->QueryInterface(cd.pUnk, &IID_IEcoLab1Events, (void**)&pIEvents);
                if ( (result == 0) && (pIEvents != 0) ) {
                    result = pIEvents->pVTbl->OnCombSortIterationStart(pIEvents, n, iteration, gap);
                    pIEvents->pVTbl->Release(pIEvents);
                }
                cd.pUnk->pVTbl->Release(cd.pUnk);
            }
 ...
 *
 * <сводка>
 *   Функция Fire_OnCombSortIterationComplete
 * </сводка>
 *
 * <описание>
 *   Функция вызова обратного интерфейса
 ...
 *
 */
static int16_t ECOCALLMETHOD CEcoLab1_Fire_OnCombSortIterationComplete(/* in */ struct IEcoLab1* me, /* in */ size_t n, /* in */ int32_t iteration, /* in */ int32_t swaps_in_iteration) {
    CEcoLab1* pCMe = (CEcoLab1*)me;
        ...
        if ( (result_fire == 0) && (pEnum != 0) ) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                ...
                if ( (result_fire == 0) && (pIEvents != 0) ) {
                    result_fire = pIEvents->pVTbl->OnCombSortIterationComplete(pIEvents, n, iteration, swaps_in_iteration);
                    pIEvents->pVTbl->Release(pIEvents);
                }
                cd.pUnk->pVTbl->Release(cd.pUnk);
            }
 ...
 *
 * <сводка>
 *   Функция Fire_OnCombSortComplete
 * </сводка>
 *
 * <описание>
 *   Функция вызова обратного интерфейса
 ...
 *
 */
static int16_t ECOCALLMETHOD CEcoLab1_Fire_OnCombSortComplete(/* in */ struct IEcoLab1* me, /* in */ size_t n, /* in */ int32_t swap_count) {
    CEcoLab1* pCMe = (CEcoLab1*)me;
    int16_t result_fire = 0;
    IEcoEnumConnections* pEnum = 0;
    IEcoLab1Events* pIEvents = 0;
    ...
    if (pCMe->m_pISinkCP != 0) {
        ...
        if ( (result_fire == 0) && (pEnum != 0) ) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                result_fire = cd.pUnk->pVTbl->QueryInterface(cd.pUnk, &IID_IEcoLab1Events, (void**)&pIEvents);
                if ( (result_fire == 0) && (pIEvents != 0) ) {
                    result_fire = pIEvents->pVTbl->OnCombSortComplete(pIEvents, n, swap_count);
                    pIEvents->pVTbl->Release(pIEvents);
                }
                cd.pUnk->pVTbl->Release(cd.pUnk);
            }
 ...
 *
 * <описание>
 *   Функция сортировки массива методом Comb sort (сортировка расческой)
 * </описание>
 *
 */
static int16_t ECOCALLMETHOD CEcoLab1_CombSort(/* in */ IEcoLab1Ptr_t me, /* in */ void* base, /* in */ size_t num, /* in */ size_t size, /* in */ int (ECOCDECLMETHOD *compare)(const void*, const void*)) {
    CEcoLab1* pCMe = (CEcoLab1*)me;
    size_t gap = num;
    int swapped = 1;
    ...
    if (num == 1) {
        CEcoLab1_Fire_OnCombSortComplete(me, num, 0);
        return ERR_ECO_SUCCESES;
    }
